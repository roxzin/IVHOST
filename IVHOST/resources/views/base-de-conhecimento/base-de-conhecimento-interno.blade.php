...
@section('titulo-pagina', 'IVHOST - ' . $base->titulo) <!-- Define o título específico desta página -->
                        ...
                        <div class="inner-wrapper">
                            <div class="meta">
                                ...
                                <div class="meta-item meta-date">
                                    <time datetime="2023-10-13T05:00:45-04:00" class="meta-info date notranslate">{{ $base->data_hora }}</time>
                                </div>
                                <div class="meta-item meta-reading">{{ $base->categoria }}</div>
                            </div>
                            <div class="hero-title-wrapper">
                                <h1 class="page-title">{{ $base->titulo }}</h1>
                            </div>
                            <div class="hero-subtext">{{ $base->sub_titulo }}</div>
                        </div>
                    </div>
                </div>
            </div>
...
<div class="row justify-content-center">
    <div class="col-md-10 col-lg-10 col-xl-9">
        <article class="blog-content content-styled">
            <div style="text-align: center;"><img src="/images/blog/{{ $base->image }}"></div>
            <p>{!! $base->texto !!}</p>
        </article>
        <b>Tags:</b>
...
<section class="block block-blogs padding-none-bottom block-align-center">
    <div class="container-fluid">
        <div class="row no-gutters">
            ...
            @foreach ($BasesRelacionados as $BasesRelacionado)
                <div class="blog-item col-md-4 col-12 animated fadeIn">
                    <div class="post">
                        <a href="/base-de-conhecimento/{{ $BasesRelacionado['titulo_slug'] }}" class="featured-image-thumbnail">
                            <div class="image" style="background-image: url('/images/blog/{{ $BasesRelacionado['image'] }}');">&nbsp;</div>
                            <div class="overlay overlay-blogs"></div>
                        </a>
                        <div class="content-wrapper match-height-row" style="min-height: 177.797px;">
                            <a href="/base-de-conhecimento/{{ $BasesRelacionado['titulo_slug'] }}" class="overlay-link"></a>
                            <div class="meta meta-top">
                                <time class="meta-info date notranslate">{{ $BasesRelacionado['data_hora'] }}</time>
                                <span class="meta-info author">{{ $BasesRelacionado['autor'] }}</span>
                            </div>
                            <h6 class="title">{{ $BasesRelacionado['titulo'] }}</h6>
                        </div>
                    </div>
                </div>
            @endforeach
